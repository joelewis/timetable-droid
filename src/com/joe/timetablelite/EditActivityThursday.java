...
public class EditActivityThursday extends Activity {
	SQLiteDatabase db;
	...
	private class UpdateDb extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... objects) {
        	...
        	for(int i=0; i < Constants.thursday.size(); i++) {
        		if(Constants.thursday.get(i).equalsIgnoreCase("+++")) {
        			values.put("period"+(i+1), "nil");	
        		}
        		else {
        			values.put("period"+(i+1), Constants.thursday.get(i));
        		}
        	}
        	db.update(TimeTableDbHelper.TABLE_NAME_TIMETABLE, values, "id = " + 4, null);
        	return null;
        }
	...
	private class QueryDb extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... objects) {
        	...
        	Constants.thursday.clear();
        	db = mDbHelper.getWritableDatabase();
        	Cursor cursor = db.query(TimeTableDbHelper.TABLE_NAME_TIMETABLE, TimeTableDbHelper.allColumns_mod, " id = " + 4, null, null, null, null);
        	cursor.moveToFirst();
        	...
        	for(int i=1; i<cursor.getColumnCount(); i++) {
        		if(cursor.getString(i).equalsIgnoreCase("nil")) {
        			Constants.thursday.add("+++");
        		} else {
        			
        			Constants.thursday.add(cursor.getString(i));
        		}
        	}
        ...
        protected void onPostExecute(final Void unused){
            ...
            //update UI with my objects
        	CustomEditAdapter adapter= new CustomEditAdapter(context, R.layout.list_view_component_edit, R.id.subject, Constants.thursday);
        	setContentView(R.layout.activity_edit);
        	final ListView lv = (ListView) findViewById(R.id.editListView);
        	lv.setAdapter(adapter);
        	listView = lv;
	...
	public void goNext(View v) {
		...
		new UpdateDb().execute();
		Intent intent = new Intent(this, EditActivityFriday.class);
		intent.putExtra("intention", 1);
		startActivity(intent);
		Constants.thursday.clear();
		//super.finish();
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		  ...
		  if (requestCode == 1) {
		     ...
		     if(resultCode == RESULT_OK){      
		         ...
		         Constants.thursday.set(periodid, result);
