...
public class EditActivityFriday extends Activity {
	SQLiteDatabase db;
	...
	private class UpdateDb extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... objects) {
        	...
        	for(int i=0; i < Constants.friday.size(); i++) {
        		if(Constants.friday.get(i).equalsIgnoreCase("+++")) {
        			values.put("period"+(i+1), "nil");	
        		}
        		else {
        		values.put("period"+(i+1), Constants.friday.get(i));
        		}
        	}
        	db.update(TimeTableDbHelper.TABLE_NAME_TIMETABLE, values, "id = " + 5, null);
        	return null;
        }
	...
	private class QueryDb extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... objects) {
        	...
        	db = mDbHelper.getWritableDatabase();
        	Cursor cursor = db.query(TimeTableDbHelper.TABLE_NAME_TIMETABLE, TimeTableDbHelper.allColumns_mod, " id = " + 5, null, null, null, null);
        	cursor.moveToFirst();
        	...
        	for(int i=1; i<cursor.getColumnCount(); i++) {
        		if(cursor.getString(i).equalsIgnoreCase("nil")) {
        			Constants.friday.add("+++");
        		} else {
        			
        			Constants.friday.add(cursor.getString(i));
        		}
        	}
        ...
        protected void onPostExecute(final Void unused){
            ...
            //update UI with my objects
        	CustomEditAdapter adapter= new CustomEditAdapter(context, R.layout.list_view_component_edit, R.id.subject, Constants.friday);
        	setContentView(R.layout.activity_edit);
        	final ListView lv = (ListView) findViewById(R.id.editListView);
        	lv.setAdapter(adapter);
        	listView = lv;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		  ...
		  if (requestCode == 1) {
		     ...
		     if(resultCode == RESULT_OK){      
		         ...
		         Constants.friday.set(periodid, result);
